(function() {
  ...
  function d3_dsvs(delimiter, mimeType) {
    function dsvs(url, callback) {
      d3.texts(url, mimeType, function(text) {
        callback(text && dsvs.parse(text));
      });
    }
    function formatRow(row) {
      return row.map(formatValue).join(delimiter);
    ...
    dsvs.parse = function(text) {
      var header;
      return dsvs.parseRows(text, function(row, i) {
        if (i) {
          var o = {}, j = -1, m = header.length;
          while (++j < m) o[header[j]] = row[j];
          return o;
    ...
    dsvs.parseRows = function(text, f) {
      function token() {
        if (reParse.lastIndex >= text.length) return EOF;
        if (eol) {
          eol = false;
...
d3.xhrs = function(url, mime, callback) {
  ...
  d3.texts = function(url, mime, callback) {
    function ready(req) {
  ...
  d3.jsons = function(url, callback) {
    d3.texts(url, "application/json", function(text) {
...
//original  var d3_category10 = [ "#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf" ];
  var d3_category10 = [ "#ff7f0e", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf" ];
